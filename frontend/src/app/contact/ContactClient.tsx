...
import { Sparkles, Mail, MapPin, Clock, CheckCircle2, AlertCircle, Loader2 } from "lucide-react";
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <Magnetic strength={0.2}>
                  <button
                    ...
                    className="flex w-full items-center justify-center gap-2 btn-primary font-semibold py-3 px-6 rounded-lg transition disabled:cursor-not-allowed disabled:opacity-60"
                  >
                ...
                {status === "success" && (
                  <p role="status" className="flex items-center gap-2 text-sm text-emerald-500 dark:text-emerald-400">
                    <CheckCircle2 className="h-4 w-4 shrink-0" aria-hidden="true" />
                    Message sent. We&apos;ll get back to you at the email you provided.
                  </p>
                )}
                {status === "error" && (
                  <p role="alert" className="flex items-center gap-2 text-sm text-red-500 dark:text-red-400">
                    <AlertCircle className="h-4 w-4 shrink-0" aria-hidden="true" />
                    Something went wrong. Please email us directly at{" "}
                    <a href={`mailto:${BUSINESS.supportEmail}`} className="underline">{BUSINESS.supportEmail}</a>.
                  </p>
                )}
                {status === "idle" && (
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    We&apos;ll respond to the email address you provide above.
                  </p>
                )}
              </form>
            </div>
