...
export function LibraryShareModal({ item, onClose, onChanged }: LibraryShareModalProps) {
  const t = useTranslations("library.shareModal");
  return (
    <Modal open={!!item} onClose={onClose} size="sm" title={t("title", { name: item?.name ?? "" })}>
    ...
    <>
      {state.data ? (
        <div className="mt-3 flex items-center gap-2">
          ...
          <button onClick={copy} aria-label={t("copyLink")} className="rounded-lg border border-border p-2 hover:bg-accent/10">
